...
focus(Client *c)
{
	...
	if (c) {
		...
		if (focusedontop && c->mon->lt[c->mon->sellt]->arrange) {
			...
			wc.stack_mode = Below; /* BYLO Below */
			...
			/* XSync(dpy, True); Dobavil CHTOBY EBUCHIJE DIALOGI NE PRIATALIS DRUG ZA DRUZHKU */
			XRaiseWindow(dpy, c->win); /* I ETO */
		}
	} else {
...
resizebarwin(Monitor *m) 
{
	...
	XMoveResizeWindow(dpy, m->barwin, m->wx, m->by, m->ww, bh); /* TUT POMENIAL POD STATUS<=>SYSTRAY */
}
...
unmanage(Client *c, int destroyed)
{
	...
	/* if (c->switchtotag)
	{
		...
		view(&a);
	} UBRAL CHTOBY NE PERESKAKIVAL FOCUS OBRATNO PRI ZAKRYTII OKNA */ 
...
updatebars(void)
{
	...
	for (m = mons; m; m = m->next) {
		...
		/* ZDES JA PEREDELAL, POTOMU CHTO SYSTREMTRAY OPIAT PLOXO SEBIA VIOL - PICOM ZAKRYVAL POLOVINU NADPISI */
		/* m->barwin = XCreateWindow(dpy, root, m->wx, m->by, w, bh, 0, DefaultDepth(dpy, screen),
		
				CopyFromParent, DefaultVisual(dpy, screen),
				CWOverrideRedirect|CWBackPixmap|CWEventMask, &wa);*/
		m->barwin = XCreateWindow(dpy, root, m->wx, m->by, w, bh, 0, DefaultDepth(dpy, screen), CopyFromParent, DefaultVisual(dpy, screen), CWOverrideRedirect|CWEventMask, &wa);
...
updatesystray(void)
{
	...
	x -= w + (TEXTW(stext) - lrpad / 2 + 2); /* TUT PEREDELAL DLIA STATUS <=> SYSTRAY */
